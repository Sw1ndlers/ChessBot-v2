...
piece_types = {
    "bb.svg": "bishop", "bk.svg": "king", "bn.svg": "knight", "bp.svg": "pawn", "bq.svg": "queen", "br.svg": "rook",
    "wb.svg": "bishop", "wk.svg": "king", "wn.svg": "knight", "wp.svg": "pawn", "wq.svg": "queen", "wr.svg": "rook"
}
...
base_path = "./pychess_pieces"
sets: Dict[PieceSetName, Dict[str, str]] = {}

def clean_svg_declaration(svg_content):
    # If the SVG contains an XML declaration, remove it
    if svg_content.startswith("<?xml"):
        # Find the first occurrence of the declaration and remove it
        xml_declaration_end = svg_content.find("?>") + 2
        svg_content = svg_content[xml_declaration_end:].lstrip()
    return svg_content

for set_name in os.listdir(base_path):
    set_path = os.path.join(base_path, set_name)
    if not os.path.isdir(set_path):
        continue
    sets[set_name] = {}
    for file_name in os.listdir(set_path):
        if file_name.lower().startswith("readme") or file_name.lower().startswith("license"):
            continue
        file_path = os.path.join(set_path, file_name)

        # Open the SVG file and parse it
        with open(file_path, "r", encoding="utf-8") as f:
            svg_content = f.read()

        svg_content = clean_svg_declaration(svg_content)
        # print(svg_content)

        # Parse SVG with lxml
        root = etree.fromstring(svg_content)

        # Determine the color and type of piece from the file name
        color = color_map[file_name[0].lower()]  # 'b' or 'w' for black or white
        piece_type = piece_types.get(file_name, None)

        if piece_type:
            # Add the id and class attributes to the root of the SVG
            root.set("id", f"{color}-{piece_type}")
            root.set("class", f"{color} {piece_type}")

            root.set("width", "50px")
            root.set("height", "50px")

            # Convert the updated SVG back to a string
            updated_svg = etree.tostring(root, pretty_print=True, encoding="unicode")

            # Store the updated SVG content in the sets dictionary
            sets[set_name][file_name] = updated_svg
        ...
        for piece_color in chess.COLORS:
            for piece_type in chess.PIECE_TYPES:
                if board.pieces_mask(piece_type, piece_color):
                    ...
                    pieceSymbol = chess.Piece(piece_type, piece_color).symbol().lower()
                    pieceColor = 'w' if piece_color == True else 'b'

                    # print(pieceSymbol, pieceColor)

                    pieceSet = sets[svgSetName][pieceColor + pieceSymbol + ".svg"]

                    defs.append(ET.fromstring(pieceSet))
